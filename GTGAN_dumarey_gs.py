...
from ctfp_tools import run_latent_ctfp_model5_3 as run_model, parse_arguments
...
def train(
    ...
    def _loss_d2(y_real, y_fake):
        ...
        # Custom loss: combined continuous (MSE) + categorical (CrossEntropy)
    def custom_loss(x_recon, x_true, num_cat_features=5, alpha=1.0):
        x_recon_cont = x_recon[:, :, :-num_cat_features]
        x_true_cont  = x_true[:, :, :-num_cat_features]
        x_recon_cat  = x_recon[:, :, -num_cat_features:]
        x_true_cat   = x_true[:, :, -num_cat_features:]
        
        # Continuous part → MSE
        loss_cont = F.mse_loss(x_recon_cont, x_true_cont)
        
        # Categorical part → CrossEntropy
        target_cat = x_true_cat.argmax(dim=-1)
        logits_cat = x_recon_cat.reshape(-1, num_cat_features)
        target_cat = target_cat.reshape(-1)
        loss_cat = F.cross_entropy(logits_cat, target_cat)
        
        return loss_cont + alpha * loss_cat
